...
class HabitFirebaseService {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  CollectionReference<Map<String, dynamic>> _habitsCollection(String userId) {
    return _firestore.collection('usuarios').doc(userId).collection('habitos');
  }

  CollectionReference<Map<String, dynamic>> _completionsCollection(String userId) {
    return _firestore.collection('usuarios').doc(userId).collection('habitos_completados');
  }
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> getHabitsQueryStream(String userId) {
    return _habitsCollection(userId).snapshots();
  }

  Stream<QuerySnapshot<Map<String, dynamic>>> getCompletionsQueryStream(String userId) {
    return _completionsCollection(userId).snapshots();
  }

  Future<void> applyHabitPatch({
  ...
  Future<void> _applyPatchTransaction({
    ...
    await _firestore.runTransaction((tx) async {
      ...
      if (!exists) {
        ...
        final initialData = _stripCloudMetadata(patch);
        updates['fieldUpdatedAt'] = _initialFieldUpdatedAtForData(initialData);
      }
